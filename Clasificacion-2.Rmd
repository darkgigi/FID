...
```{r}
set.seed(42)
knn_model_Hdalc <- train(Hdalc ~ ., data = df, method = "knn", trControl = train_control, tuneGrid = expand.grid(k = 1:10))  # Ajuste de k entre 1 y 10
knn_model_Hwalc <- train(Hwalc ~ ., data = df, method = "knn", trControl = train_control, tuneGrid = expand.grid(k = 1:10))  # Ajuste de k 
...
# Entrenar el modelo SVM radial con ajuste manual
set.seed(42)
...
# Entrenar el modelo SVM radial con validación cruzada
set.seed(42)
cart_model_Hdalc <- train(Hdalc ~ ., data = df, method = "rpart", trControl = train_control,   # Configuración de k-fold
                    tuneLength = 10)  # Ajuste de hiperparámetros
cart_model_Hwalc <- train(Hwalc ~ ., data = df, method = "rpart", trControl = train_control,   # Configuración de k-fold
                    tuneLength = 10)  # Ajuste de hiperparámetros
# Mostrar el mejor modelo
